...
$image_upload_grid = NULL; // Initialize page object first

class cimage_upload_grid extends cimage_upload {
	...
	// Page object name
	var $PageObjName = 'image_upload_grid';

	// Grid form hidden field names
	var $FormName = 'fimage_uploadgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["image_upload"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if (!ew_Empty($this->location->Upload->Value))
			return FALSE;
	...
	function LoadDefaultValues() {
		...
		$this->location->Upload->Index = $this->RowIndex;
	...
	function LoadFormValues() {
		...
		$this->date_create->setOldValue($objForm->GetValue("o_date_create"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->image_id->CurrentValue = strval($arKeys[0]); // image_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
		...
		$this->image_id->ViewCustomAttributes = "";
			...
			if (!ew_Empty($this->location->CurrentValue))
					if ($this->RowIndex == '$rowindex$')
						$this->location->Upload->FileName = "";
					else
						$this->location->Upload->FileName = $this->location->CurrentValue;
			if (is_numeric($this->RowIndex) && !$this->EventCancelled) ew_RenderUploadField($this->location, $this->RowIndex);
