...
$image_upload_view = NULL; // Initialize page object first

class cimage_upload_view extends cimage_upload {
	...
	// Page object name
	var $PageObjName = 'image_upload_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "image_uploadlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "image_uploadlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "image_uploadlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($image_upload_view)) $image_upload_view = new cimage_upload_view();

// Page init
$image_upload_view->Page_Init();

// Page main
$image_upload_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$image_upload_view->Page_Render();
?>
...
var CurrentForm = fimage_uploadview = new ew_Form("fimage_uploadview", "view");

// Form_CustomValidate event
fimage_uploadview.Form_CustomValidate = 
...
fimage_uploadview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $image_upload_view->ExportOptions->Render("body") ?>
<?php
	foreach ($image_upload_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $image_upload_view->ShowPageHeader(); ?>
<?php
$image_upload_view->ShowMessage();
?>
<form name="fimage_uploadview" id="fimage_uploadview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($image_upload_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $image_upload_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="image_upload">
<input type="hidden" name="modal" value="<?php echo intval($image_upload_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($image_upload->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_image_upload_id"><?php echo $image_upload->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $image_upload->id->CellAttributes() ?>>
<span id="el_image_upload_id">
<span<?php echo $image_upload->id->ViewAttributes() ?>>
<?php echo $image_upload->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($image_upload->location->Visible) { // location ?>
	<tr id="r_location">
		<td class="col-sm-2"><span id="elh_image_upload_location"><?php echo $image_upload->location->FldCaption() ?></span></td>
...
<?php echo ew_GetFileViewTag($image_upload->location, $image_upload->location->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($image_upload->date_create->Visible) { // date_create ?>
	<tr id="r_date_create">
		<td class="col-sm-2"><span id="elh_image_upload_date_create"><?php echo $image_upload->date_create->FldCaption() ?></span></td>
...
<?php echo $image_upload->date_create->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($image_upload->blog_id->Visible) { // blog_id ?>
	<tr id="r_blog_id">
		<td class="col-sm-2"><span id="elh_image_upload_blog_id"><?php echo $image_upload->blog_id->FldCaption() ?></span></td>
...
<?php echo $image_upload->blog_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fimage_uploadview.Init();
</script>
<?php
$image_upload_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$image_upload_view->Page_Terminate();
?>
