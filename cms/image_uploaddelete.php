...
$image_upload_delete = NULL; // Initialize page object first

class cimage_upload_delete extends cimage_upload {
	...
	// Page object name
	var $PageObjName = 'image_upload_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("image_uploadlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in image_upload class, image_uploadinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("image_uploadlist.php"); // Return to list
			}
		}
	}
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$OldFiles = ew_Empty($row['location']) ? array() : array($row['location']);
				$OldFileCount = count($OldFiles);
				for ($i = 0; $i < $OldFileCount; $i++) {
					if (file_exists($this->location->OldPhysicalUploadPath() . $OldFiles[$i]))
...
// Create page object
if (!isset($image_upload_delete)) $image_upload_delete = new cimage_upload_delete();

// Page init
$image_upload_delete->Page_Init();

// Page main
$image_upload_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$image_upload_delete->Page_Render();
?>
...
var CurrentForm = fimage_uploaddelete = new ew_Form("fimage_uploaddelete", "delete");

// Form_CustomValidate event
fimage_uploaddelete.Form_CustomValidate = 
...
fimage_uploaddelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $image_upload_delete->ShowPageHeader(); ?>
<?php
$image_upload_delete->ShowMessage();
?>
<form name="fimage_uploaddelete" id="fimage_uploaddelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($image_upload_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $image_upload_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="image_upload">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($image_upload_delete->RecKeys as $key) { ?>
...
<?php if ($image_upload->id->Visible) { // id ?>
		<th class="<?php echo $image_upload->id->HeaderCellClass() ?>"><span id="elh_image_upload_id" class="image_upload_id"><?php echo $image_upload->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($image_upload->location->Visible) { // location ?>
		<th class="<?php echo $image_upload->location->HeaderCellClass() ?>"><span id="elh_image_upload_location" class="image_upload_location"><?php echo $image_upload->location->FldCaption() ?></span></th>
<?php } ?>
<?php if ($image_upload->date_create->Visible) { // date_create ?>
		<th class="<?php echo $image_upload->date_create->HeaderCellClass() ?>"><span id="elh_image_upload_date_create" class="image_upload_date_create"><?php echo $image_upload->date_create->FldCaption() ?></span></th>
<?php } ?>
<?php if ($image_upload->blog_id->Visible) { // blog_id ?>
		<th class="<?php echo $image_upload->blog_id->HeaderCellClass() ?>"><span id="elh_image_upload_blog_id" class="image_upload_blog_id"><?php echo $image_upload->blog_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$image_upload_delete->RecCnt = 0;
$i = 0;
while (!$image_upload_delete->Recordset->EOF) {
	$image_upload_delete->RecCnt++;
	$image_upload_delete->RowCnt++;

	// Set row properties
	$image_upload->ResetAttrs();
	$image_upload->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$image_upload_delete->LoadRowValues($image_upload_delete->Recordset);

	// Render row
	$image_upload_delete->RenderRow();
?>
	<tr<?php echo $image_upload->RowAttributes() ?>>
<?php if ($image_upload->id->Visible) { // id ?>
		<td<?php echo $image_upload->id->CellAttributes() ?>>
<span id="el<?php echo $image_upload_delete->RowCnt ?>_image_upload_id" class="image_upload_id">
...
<span id="el<?php echo $image_upload_delete->RowCnt ?>_image_upload_location" class="image_upload_location">
...
<span id="el<?php echo $image_upload_delete->RowCnt ?>_image_upload_date_create" class="image_upload_date_create">
...
<?php } ?>
<?php if ($image_upload->blog_id->Visible) { // blog_id ?>
		<td<?php echo $image_upload->blog_id->CellAttributes() ?>>
<span id="el<?php echo $image_upload_delete->RowCnt ?>_image_upload_blog_id" class="image_upload_blog_id">
...
<?php
	$image_upload_delete->Recordset->MoveNext();
}
$image_upload_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $image_upload_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fimage_uploaddelete.Init();
</script>
<?php
$image_upload_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$image_upload_delete->Page_Terminate();
?>
