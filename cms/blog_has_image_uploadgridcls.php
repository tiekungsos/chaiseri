...
$blog_has_image_upload_grid = NULL; // Initialize page object first

class cblog_has_image_upload_grid extends cblog_has_image_upload {
	...
	// Page object name
	var $PageObjName = 'blog_has_image_upload_grid';

	// Grid form hidden field names
	var $FormName = 'fblog_has_image_uploadgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["blog_has_image_upload"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->blog_id->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_blog_id") && $objForm->HasValue("o_blog_id") && $this->blog_id->CurrentValue <> $this->blog_id->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->blog_id->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->blog_id->setOldValue($objForm->GetValue("o_blog_id"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->blog_id->CurrentValue = strval($arKeys[0]); // blog_id
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "image_upload") {
			...
			if ($GLOBALS["image_upload"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "blog") {
			$this->blog_id->Visible = FALSE;
			if ($GLOBALS["blog"]->EventCancelled) $this->EventCancelled = TRUE;
		}
